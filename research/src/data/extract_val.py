...
def main():
    ...
    for category in categories:
        ...
        if not os.path.exists(val_json_path):
            print(f"Bỏ qua {category} vì không tìm thấy file {val_json_path}")
            continue
            
        print(f"\n1. Đang nạp {val_json_path}...")
        ...
        print(f"2. Bắt đầu trích xuất {len(data)} truy vấn cho {category}...")
        
        with torch.no_grad():
            for idx, item in enumerate(tqdm(data, desc=f"Extracting {category}")):
        ...
        hidden_path = f"data/features/{category}_val_text_hidden.pt"
        embed_path = f"data/features/{category}_val_text_embeds.pt"
        
        torch.save(hidden_dict, hidden_path)
        torch.save(embed_dict, embed_path)
        
        print(f"3. Hoàn tất {category}!")
